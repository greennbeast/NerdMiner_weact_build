...
// Get per-worker hash rates in KH/s
void getWorkerHashRates(float rates[], int max_workers) {
  static uint32_t last_worker_hashes[8] = {0};
...
// TEST: https://bitcoin.stackexchange.com/questions/22929/full-example-data-for-scrypt-stratum-client
  ...
  while(true) {
    ...
    if(!isMinerSuscribed)
    {
      ...
      // STEP 2: Pool authorize each worker with unique suffix (.w0..wN-1)
      for (int i = 0; i < MINER_WORKER_COUNT; ++i) {
        char user_buf[128];
        snprintf(user_buf, sizeof(user_buf), "%s.w%d", mWorker.wName, i);
        tx_mining_auth(client, user_buf, mWorker.wPass);
    ...
    while (!job_result_list.empty())
    {
      ...
      if (res->worker_id < 8) {
        worker_hashes[res->worker_id] += res->nonce_count;
        worker_last_update[res->worker_id] = millis();
      }
      if (res->difficulty > currentPoolDifficulty && job_pool == res->id && res->nonce != 0xFFFFFFFF)
      {
        ...
        // Create unique worker name with worker ID suffix
        mining_subscribe workerWithId = mWorker;
        char suffix[8];
        snprintf(suffix, sizeof(suffix), ".w%d", res->worker_id);
        strncat(workerWithId.wName, suffix, sizeof(workerWithId.wName) - strlen(workerWithId.wName) - 1);
        tx_mining_submit(client, workerWithId, mJob, res->nonce, sumbit_id);
...
void runMonitor(void *name)
{
  ...
  while (1)
  {
    ...
    { 
      ...
      // Auto-rotate screens periodically (keeps UI cycling without button presses)
      autoRotateScreensIfNeeded(now_millis);
