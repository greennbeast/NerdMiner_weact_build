...
void setup()
{
  ...
  // Ensure CPU is running at max supported frequency (ESP32: 240MHz)
  // Overclock beyond 240MHz is not supported by Arduino core and may be unstable.
  ...
  /******** CREATE MINER TASKS *****/
  // Spawn miner worker tasks according to MINER_WORKER_COUNT (defined in platformio.ini)
  {
    int workerCount = MINER_WORKER_COUNT;
    int cores = (SOC_CPU_CORES_NUM >= 2) ? 2 : 1;
    for (int i = 0; i < workerCount; ++i)
    {
      char tname[16];
      snprintf(tname, sizeof(tname), "Miner-%d", i);
      TaskHandle_t th = NULL;
  #ifdef HARDWARE_SHA265
      #if defined(CONFIG_IDF_TARGET_ESP32)
        xTaskCreatePinnedToCore(minerWorkerHw, tname, 3584, (void*)(uint32_t)i, 3, &th, i % cores);
      #else
        xTaskCreatePinnedToCore(minerWorkerHw, tname, 4096, (void*)(uint32_t)i, 3, &th, i % cores);
      #endif
  #else
      #if defined(CONFIG_IDF_TARGET_ESP32)
        xTaskCreatePinnedToCore(minerWorkerSw, tname, 5000, (void*)(uint32_t)i, 1, &th, i % cores);
      #else
        xTaskCreatePinnedToCore(minerWorkerSw, tname, 6000, (void*)(uint32_t)i, 1, &th, i % cores);
      #endif
  #endif
      if (th)
        esp_task_wdt_add(th);
